...
router.delete('/deleteBoard', authenticateToken, async (req, res) => {
    try {
        ...
        if (!isAdmin) {
            return res.status(403).send( 'You do not have permission to delete the board.' );
        }
...
router.post('/addMember', authenticateToken, async (req, res) => {
    try {
        ...
        if (await BoardMember.findOne({ user: user_to_add._id, board: board._id})) {
        ...
        const newBoardMember = new BoardMember({user: user_to_add._id, board: board._id, isAdmin: false});
        const savedBoardMember = await newBoardMember.save();
